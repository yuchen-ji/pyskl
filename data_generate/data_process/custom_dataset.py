...
# # Step 1：导入标签文件和视频文件，
# train = load('data_generate/train.json')
# test = load('data_generate/test.json')
# tmpl = 'data_generate/video__/{}.mp4'

# lines = [(tmpl + ' {}').format(x['vid_name'], x['label']) for x in train + test]
# mwlines(lines, 'custom.list')
...
"""
# bash tools/dist_run.sh tools/data/custom_2d_skeleton.py 6 --video-list data_generate/custom.list --out data_generate/custom_annos.pkl

# # Step 3: 合并训练集和测试集为一个pickle文件
# train = load('data_generate/train.json')
# test = load('data_generate/test.json')
# annotations = load('data_generate/custom_annos.pkl')
# split = dict()
# split['train'] = [x['vid_name'] for x in train]
# split['test'] = [x['vid_name'] for x in test]
# dump(dict(split=split, annotations=annotations), 'data_generate/custom_hrnet.pkl')  # 最终的pickle文件，用于训练行为识别模型