...
def parse_args():
    ...
    parser.add_argument(
        '--label-map',
        default='workspace/label_map/factory6.txt',
        help='label map file')
    parser.add_argument(
...
def main():
    ...
    if GCN_flag:
        ...
        keypoint_score = 0.8 * np.ones_like(keypoint_score)
        if True:
    ...
    for item in range(len(vis_frames)):
        cv2.putText(vis_frames[item], label_list[item], (10, 30), FONTFACE, FONTSCALE,
                    FONTCOLOR, THICKNESS, LINETYPE)

    vid = mpy.ImageSequenceClip([x[:, :, ::-1] for x in vis_frames], fps=15)
...
def slide_window(fake_anno, keypoint, keypoints_score):
    ...
    # len(keypoint[0]) + 1, 其中 +1 是因为数组切片的时候属于 [) 区间, 所以 index 可以多一位。
    for item in range(start_frame, len(keypoint[0]) + 1, iter_frame): 
        if item - tmp_frame < 25 and item != start_frame:
